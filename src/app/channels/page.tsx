      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        <section className="rounded-2xl border border-primary/20 bg-primary/5 p-6 sm:p-8">
          <div className="max-w-2xl">
            ...
            <p className="mt-2 text-muted-foreground">Connect your accounts through Zernio and manage every conversation from Bookly. Your API key keeps the connection secure.</p>
        ...
        <div className="mt-10 flex items-end justify-between gap-4">
          <div><h2 className="text-xl font-semibold tracking-tight">Available channels</h2><p className="mt-1 text-sm text-muted-foreground">All channels supported by your Zernio backend.</p></div>
          ...
          {MESSAGE_PLATFORMS.map((platform) => (
            <article key={platform} className="flex min-h-52 flex-col rounded-xl border border-[var(--chat-border)] bg-card p-5 shadow-sm transition-shadow hover:shadow-md">
              <div className="flex items-start justify-between gap-4">
                ...
                <span className="inline-flex items-center gap-1.5 text-xs font-medium text-muted-foreground"><Check className="size-3.5 text-emerald-500" />Zernio supported</span>
              </div>
              ...
              <Button asChild variant="outline" className="mt-5 w-full bg-background"><a href="https://zernio.com" target="_blank" rel="noreferrer">Add {PLATFORM_LABELS[platform]}</a></Button>
            </article>
          ))}
        </section>
