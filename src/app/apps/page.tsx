...
export default function AppsPage() {
  ...
  return (
    <main className="h-dvh overflow-y-auto overscroll-contain bg-[var(--chat-canvas)] text-foreground">
      <header className="sticky top-0 z-10 border-b border-[var(--chat-border)] bg-background/95 backdrop-blur"><div className="mx-auto flex min-h-16 max-w-6xl items-center justify-between gap-4 px-4 sm:px-6 lg:px-8"><div><h1 className="text-lg font-semibold tracking-tight">Apps</h1><p className="hidden text-sm text-muted-foreground sm:block">Connect your calendar and favorite tools with Composio.</p></div><span className="inline-flex items-center gap-2 text-sm text-muted-foreground"><Plug className="size-4 text-primary" />Composio</span></div></header>
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
        <section className="rounded-2xl border border-primary/20 bg-primary/5 p-6 sm:p-8"><div className="flex max-w-2xl gap-4"><div className="flex size-11 shrink-0 items-center justify-center rounded-xl bg-primary text-primary-foreground"><AppWindow className="size-5" /></div><div><h2 className="text-2xl font-semibold tracking-tight">Connect your work tools</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">Composio securely connects Bookly to Google Calendar, email, CRM, and productivity apps so your meeting bot can check availability and schedule for you.</p></div></div></section>
        <div className="mt-8 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="text-xl font-semibold tracking-tight">App directory</h2><p className="mt-1 text-sm text-muted-foreground">Every toolkit available to your Composio project.</p></div><div className="relative w-full sm:w-72"><Search className="pointer-events-none absolute top-1/2 left-3 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search apps" className="pl-9" /></div></div>
